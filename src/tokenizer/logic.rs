use crate::tokenizer::{
    Block, Delimiter, Grouping, InitOnce, Keyword, Literal, Token, TokenStream, DELIMITERS,
    GROUPINGS, KEYWORDS, SKIP,
};
...
impl<'a> Block<'a> {
    fn parse(input: &'a str, offset: usize) -> Result<(usize, Self), ()> {
        let (_, g) = Grouping::parse_open(input)?;
        let (i, s) = TokenStream::parse(&input[1..], offset + 1)?;
        ...
        while index < input.len() {
            match input[index..].chars().nth(0).unwrap() {
                ...
                _ if closing_grouping_follows(&input[index..]) => {
                    return Ok((index, Self::from(out, offset)));
                }
                _ if opening_grouping_follows(&input[index..]) => {
                    let (i, n) = Block::parse(&input[index..], offset + index)?;
...
pub fn tokenize<'a>(input: &'a str) -> Result<(usize, TokenStream<'a>), ()> {
    TokenStream::parse(input, 0)
}
